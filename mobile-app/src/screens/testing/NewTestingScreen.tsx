...
const NewTestingScreen: React.FC = () => {
  ...
  const handleAnalyze = () => {
    ...
    selectedMarkets.forEach(market => {
      if (rulesForProduct[market]) {
        rulesForProduct[market].forEach((t: any) => {
          tests.push({ ...t, market });
        });
      } else {
        tests.push({ code: `TEST_${market.toUpperCase().replace(/\s+/g, '_')}`, name: `${market} Standard Lab Testing`, market });
      }
    });
  ...
  const handleSubmit = () => {
    ...
    navigation.navigate('UploadTestingDocuments', {
      productName,
      modelNumber,
      description,
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, alignItems: 'center' }}>
          <Text style={styles.headerTitle}>New Testing Request</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
          ...
          <View>
            <View style={[styles.infoBox, { marginBottom: 24 }]}>
              ...
              <Text style={styles.infoText}>
                Select your product and target markets. Our engine will automatically determine the specific Lab Testing standards required.
              </Text>
            </View>
            ...
            >
              <View style={{ flex: 1 }}>
                {selectedProduct ? (
                  ...
                  <Text style={styles.selectorPlaceholder}>Search from 1000+ products...</Text>
                )}
              </View>
            ...
            {/* MARKETS SELECTOR */}
            <Text style={[styles.sectionLabel, { marginTop: 32 }]}>2. Target Markets</Text>
            <TouchableOpacity 
            ...
            <Button title="Analyze Testing Requirements" onPress={handleAnalyze} size="lg" style={{ marginTop: 40 }} icon={<Ionicons name="analytics-outline" size={20} color="#FFFFFF" />} />
          </View>
        )}
        ...
        {step === 2 && (
          <View>
            <Text style={styles.sectionLabel}>Required Lab Tests</Text>
            <View style={styles.recommendationList}>
              {recommendedTests.map((test, idx) => (
                <View key={idx} style={[styles.certCard, { backgroundColor: isDark ? colors.bgCardLight : '#FFF' }]}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.certName}>{test.name}</Text>
                    <Text style={styles.certCode}>{test.code}</Text>
                  </View>
                  <Badge label={test.market} variant="info" size="sm" />
                </View>
              ))}
            </View>
            ...
            <View style={[styles.formCard, { marginTop: 32, backgroundColor: isDark ? colors.bgCard : '#FFF' }]}>
              ...
              <Input label="Model Number" value={modelNumber} onChangeText={setModelNumber} placeholder="e.g. BB-3000X" />
              <Input label="Description" value={description} onChangeText={setDescription} placeholder="Brief product description for the Lab..." multiline numberOfLines={3} style={{ height: 80, textAlignVertical: 'top' }} />
            </View>

            <Button title="Proceed to Upload Specs" onPress={handleSubmit} size="lg" style={{ marginTop: 24 }} icon={<Ionicons name="arrow-forward" size={20} color="#FFFFFF" />} />
          </View>
        )}
...
const makeStyles = (colors: any, isDark: boolean) => StyleSheet.create({
  ...
  certIconWrap: { width: 40, height: 40, borderRadius: 20, backgroundColor: isDark ? 'rgba(108, 99, 255, 0.1)' : 'rgba(108, 99, 255, 0.1)', alignItems: 'center', justifyContent: 'center', marginRight: 16 },
...
export default NewTestingScreen;
