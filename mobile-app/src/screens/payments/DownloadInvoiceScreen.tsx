...
// Seller GST number is a business/config value. Set via app config before
// presenting a compliant GST invoice; until then the row is hidden rather than
// showing a fabricated number. (OPS: provide the real GSTIN.)
const SELLER_GSTIN = '';
...
const DownloadInvoiceScreen: React.FC = () => {
  ...
  const billToName = user?.companyName || user?.name || 'Customer';
  const billToGstin = (user as any)?.gstNumber || '';

  const invoiceNo = tx.appNo && tx.appNo !== 'N/A' ? `INV-${tx.appNo}` : `INV-${tx.id}`;
  const gstAmount = Math.round(tx.amount * 0.18);
  const baseAmount = tx.amount - gstAmount;
  const invoiceDate = tx.date;

  // The authoritative invoice is the PDF generated by the backend (S3). Open it
  // when available instead of faking a local download.
  const handleDownload = async () => {
    if (tx.invoice_url) {
      const ok = await Linking.canOpenURL(tx.invoice_url).catch(() => false);
      if (ok) return Linking.openURL(tx.invoice_url);
    }
    Alert.alert('Invoice not ready', 'Your invoice PDF is not available yet. It is generated once the payment is confirmed.');
  };

  const handleShare = async () => {
    try {
      const link = tx.invoice_url ? `\n\nInvoice PDF: ${tx.invoice_url}` : '';
      await Share.share({
        message: `DICE by Sanyog — Invoice ${invoiceNo}\nAmount: ₹${tx.amount.toLocaleString('en-IN')}\nDate: ${invoiceDate}${link}`,
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={[styles.invoiceCard, Shadows.md]}>
          <LinearGradient colors={isDark ? [colors.bgCard, colors.bgCardLight] : ['#FFFFFF', '#FAFBFF']} style={styles.invoiceCardInner}>
            ...
            {/* Bill To — from the signed-in customer, not fabricated */}
            <Text style={styles.sectionLabel}>BILL TO</Text>
            <Text style={styles.billName}>{billToName}</Text>
            {billToGstin ? <Text style={styles.billDetail}>GSTIN: {billToGstin}</Text> : null}
            {SELLER_GSTIN ? <Text style={styles.billDetail}>Seller GSTIN: {SELLER_GSTIN}</Text> : null}
