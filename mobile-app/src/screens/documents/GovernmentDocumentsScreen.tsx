...
const GovernmentDocumentsScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const insets = useSafeAreaInsets();
  const { colors, isDark } = useTheme();
  const { items: DOCS, loading, refreshing, error, refresh } = useDocuments('government');
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState icon={error ? 'cloud-offline-outline' : 'document-outline'} title={error ? 'Could not load' : 'No Government Documents'} subtitle={error ?? 'Upload relevant documents to get started.'} />
        }
        refreshControl={
...
export default GovernmentDocumentsScreen;
