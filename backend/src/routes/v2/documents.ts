...
import { documentService, UploadAuthorizationError } from '../../services/documentService';
...
router.post('/presigned-url', async (req: AuthRequest, res: Response) => {
  ...
  try {
    const result = await documentService.presignUpload({
      ...
      is_staff: STAFF_ROLES.includes(req.user!.role as any),
      filename,
      mime_type: mimeType,
    ...
    return res.json({ data: { uploadUrl: result.url, key: result.s3_key, upload_id: result.upload_id, publicUrl: result.url } });
  } catch (err: any) {
    if (err instanceof UploadAuthorizationError) return res.status(403).json({ error: 'upload_not_authorized', message: err.message });
...
router.post('/finalize', validate(finalizeSchema), async (req: AuthRequest, res: Response) => {
  const { s3_key, upload_id, name, doc_type, mime_type, size_bytes, sha256, document_id, application_id, change_reason, description, tags } = req.body;
  ...
  try {
    const { document, version } = await documentService.finalizeUpload({
      ...
      user_id: req.user!._id as any,
      is_staff: STAFF_ROLES.includes(req.user!.role as any),
      s3_key, upload_id, name, doc_type, mime_type, size_bytes, sha256,
      document_id: document_id ? new Types.ObjectId(document_id) : undefined,
    ...
    // H1: an object the caller was never allocated is an authorization failure.
    if (err instanceof UploadAuthorizationError) return res.status(403).json({ error: 'upload_not_authorized', message: err.message });
    return res.status(400).json({ error: err.message });
  }
});
